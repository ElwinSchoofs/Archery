 ...
 */
public class Archer {
    private final static int START_COUNTER = 135788;
    ...
    private final static int WEIGHTED_MISS = 7;
    ...
    private Archer(String firstName, String lastName) {
        if (idCounter < START_COUNTER) {
            idCounter = START_COUNTER;
        }
        this.id = idCounter;
        idCounter++;
    ...
    public void registerScoreForRound(int round, int[] points) {
        for (int i = 0; i < MAX_ARROWS; i++) {
            this.pointsPerRound[round][i] = points[i];
        }
    }
    ...
    public int getWeightedScore() {
        ...
        for (int i = 0; i < MAX_ROUNDS; i++) {
            for (int j = 0; j < MAX_ARROWS; j++) {
                if (pointsPerRound[i][j] == MISS) {
                    totalWeightedScore -= WEIGHTED_MISS;
                } else {
                    totalWeightedScore += pointsPerRound[i][j] + 1;
                }
            }
        }
